...
```perl
my @in = map { $_ eq '>' } grep {/[<>]/}split //, <$fh>;
my($bs,$bl,@r) = ( 511, 257, [60],[16,56,16],[8,8,56],[32,32,32,32],[48,48] );
my($rn,@t,@b, @h) = (0,$bs);
...
while( 1 ) {
  ...
  while(1) {
    ...
    if( $d ) {
      map { $_>>=1 } @r unless grep { $t[-$y-$_] & ($r[$_]>>1) } 0 .. $#r;
    } else {
      map { $_<<=1 } @r unless grep { $t[-$y-$_] & ($r[$_]<<1) } 0 .. $#r;
    }
    if( grep { $t[-$y-$_-1] & $r[$_] } 0 .. $#r ) {
      $t[-$y-$_] |= $r[$_] for 0..$#r;
      last;
    }
    $y++;
