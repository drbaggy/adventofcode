...
my%M=map{$_->[2],$_}@D;my%F=map{$_=>1}qw(.12);
...
$Q=0;for(0..$#V){my($x,$y)=@{$V[$_]};next if$M[$y][$x]eq'#';
my($X,$Y)=($x+$M{$M[$y][$x]}[1],$y+$M{$M[$y][$x]}[0]);
if($M[$Y][$X]ne'1'){($M[$Y][$X],$M[$y][$x])=('.','#');next}
$Q=1;($M[$Y][$X],$M[$y][$x])=('#','.');$V[$_]=[$X,$Y]}
if(++$t==10){my($r,$b,$l,$z)=(0,0,$W,$H);for(@V){
$l=$_->[0]if$_->[0]<$l;$b=$_->[1]if$_->[1]>$b;
$r=$_->[0]if$_->[0]>$r;$z=$_->[1]if$_->[1]<$z}say-@V+(1+$b-$z)*($r-$l+1)}
push@D,shift@D}
say$t
