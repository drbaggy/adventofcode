...
**[Home](README.md)** - **[Description](#description)** - **Files:** [Solution to day 25](25.pl), [Input for day 25](data/25.txt), [Output for day 25](out/25.txt).

## Solution

A nice easy Christmas day problem.

We create two lookup tables from symbols to decimal (`%M`) and one from decimals to symbols (`%R1`);

We then loop through the lines in the file - computing the decimal versions of each string, we use the multiply (shift) and add method to convert the bases. At we loop through the symbols we multiply by `5`, and add the score for the symbol. We then sum these. For a bit of golf here we use map and take the last element to get the answer.

Once we have our sum we have to do the reverse to get it back into SNAFU units. We get the *unit*  by adding 2 taking the remainder base 5 and subtracting 2 and grabbing the symbol. We then also reduce the value by subtracting the number before and dividing by 5.
...
Your puzzle answer was `2=001=-2=--0212-22-2`.
