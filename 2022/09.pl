...
## Slurp instructions into array as will be using
## twice...
...
## We want to solve the problem for a rope with 2
## knots (part 1) && 10 knots (part 2). $K is the
## number of non-head knots (1 & 9 respectively)
for my $K ( 1, 9 ) {  ## 2 & 10 knots

## Define the knot positions - all start at the origin.
  my @p = map { [0,0] } 0.. $K;

## Create a record of where the tail has been. We
## have to include the origin as we only record
## the position of it once it has moved...
...
## Move the head one square at a time along the line
## given
    O: for (1..$l) {
      ( $p[0][0] += $v{$d}[0], $p[0][1] += $v{$d}[1] );

## Loop through knots, if knot is adjacent to previous
## knot - it doesn't move - also means other knots do
## not move....
