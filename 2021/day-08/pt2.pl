...
while( my $line = <> ) {
  ...
  ## letters in each array into alphabetic order.

  chomp $line;
  my @in  = map { join '', sort split // } split /[\s|]+/, $line;
  my @out = splice @in,-4,4;

  ## First job create an array of arrays where the array is keyed on the
  ## length of the string and the values is an array of strings of the
  ## given length. To avoid an error in the digits line below we
  ## initialise the array for length 0...
  ...
  ## 1 has length 2, 7 has length 3, 4 has length 4 and 8 has length 7.

  my @digits = map { $len[$_][0] } qw(0 2 0 0 4 0 0 3 7 0);

  ## Next we will use regular expressions to identify the other digits.
  ...
  $digits[ /$re4/ ? 9 : /$re1/    ? 0 : 6 ] = $_ for @{$len[6]};

  ## Finally we do the 5-characther strings 2,3,5.
  ## Of these the only one that "contains" the number 7 is 3...
  ## But we need another regex to distinguish between 2 + 3. We note that
  ## the top right vertical bar is only in 2 - and is the only one not in
  ...
  ## the total and then we are good to go!
